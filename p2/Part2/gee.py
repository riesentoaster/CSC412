...
class BinaryExpr( Expression ):
	...
	def tipe(self, tm):
		...
		if (self.left.tipe(tm) != self.right.tipe(tm) and 'errorMessage' not in tm.keys()):
			raise Exception(f"Type Error: {self.left.tipe(tm)} != {self.right.tipe(tm)}!")
		if (self.op in booleanOperators):
			return 'boolean'
		else:
			return self.left.tipe(tm)
...
class Assign(Expression):
	...
	def tipe(self, tm):
		if (str(self.left) not in tm.keys()):
			print(f"{str(self.left)} {self.right.tipe(tm)}")
			return {**tm, str(self.left): self.right.tipe(tm)}
		else:
			if (tm[str(self.left)] != self.right.tipe(tm)):
				raise Exception(f"Type Error: {tm.get(str(self.left))} = {self.right.tipe(tm)}!")
			else:
				return tm
...
class Ident( Expression ):
	...
	def tipe(self, tm):
		try:
			return tm[str(self.name)]
		except:
			raise Exception(f"Type Error: {self.name} is referenced before being defined!")
...
class WhileStmt():
	...
	def tipe(self, tm):
		if (self.expr.tipe(tm) != 'boolean'):
			raise Exception('While statements needs boolean as expression.')
		return self.block.tipe(tm)
...
class IfStmt():
	...
	def tipe(self, tm):
		if (self.expr.tipe(tm) != 'boolean'):
			raise Exception('If statements needs boolean as expression.')
		if (self.block2):
			tm = self.block1.tipe(tm)
			return self.block2.tipe(tm)
		else:
			return self.block1.tipe(tm)
